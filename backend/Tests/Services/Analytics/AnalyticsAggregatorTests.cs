...
public class AnalyticsAggregatorTests
{
    ...
    public async Task GetSummaryAsync_AggregatesAllProvidersCorrectly_OnSuccess()
    {
        ...
        _mockJobsOverTimeProvider.Setup(p => p.GetWeeklyJobsAsync(It.IsAny<CancellationToken>())).ReturnsAsync(jobsOverTimeResult);
        _mockHistogramProvider.Setup(p => p.GetHistogramScoresAsync(It.IsAny<CancellationToken>())).ReturnsAsync(histogramResult);
    ...
    public async Task GetSummaryAsync_ReturnsFailure_WhenAnyProviderThrowsException()
    {
        ...
        // Assert
        Assert.False(result.IsSuccess);
        Assert.Equal("An error occurred while retrieving the analytics summary.", result.ErrorMessage);
        Assert.Null(result.Data);
    }
}
