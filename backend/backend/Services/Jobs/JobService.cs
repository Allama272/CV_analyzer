...
public class JobService : IJobService
{
    ...
    public JobService(ResumeDbContext dbContext, IBackgroundJobClient backgroundJobClient)
    {
    ...
    public async Task<ServiceResult<UserJob?>> SaveJobAsync(JobSentDto jobSent, string userId)
    {
        ...
        // bool jobExists = await _dbContext.UserJobs
        //     .AnyAsync(uj => uj.UserId == userId && uj.Company == jobSent.Company && uj.JobTitle == jobSent.JobTitle);
        //
        // if (jobExists)
        // {
        //     return ServiceResult<UserJob>.Failure("You have already saved this job.");
        // }
    ...
    public async Task<ServiceResult<JobWithFeedbacksPreview?>> GetJobByIdAsync(int jobId, string userId)
    {
        var job = await _dbContext.UserJobs.Where(uj => uj.Id == jobId && uj.UserId == userId)
    ...
    public async Task<ServiceResult<int?>> AnalyzeResumeToJob(int resumeId, int jobId, string userId)
    {
        // ownership verification
        var resumeExist = await _dbContext.Resumes.AnyAsync(r => r.Id == resumeId && r.UserId == userId);
        ...
        var existingFeedback = await _dbContext.ResumeJobFeedbacks
            .FirstOrDefaultAsync(r => r.ResumeId == resumeId && r.UserJobId == jobId);
        if (existingFeedback != null)
        {
            ...
            await _dbContext.SaveChangesAsync();
            _backgroundJobClient.Enqueue<IAtsService>(service => service.AnalyzeJobResume(existingFeedback.Id));
        ...
        catch (DbUpdateException e)
        {
            ...
            var raceConditionId = await _dbContext.ResumeJobFeedbacks
                .Where(r => r.ResumeId == resumeId && r.UserJobId == jobId)
                .Select(r => (int?)r.Id)
                .FirstOrDefaultAsync();
        ...
        _backgroundJobClient.Enqueue<IAtsService>(service => service.AnalyzeJobResume(feedbackJob.Id));
        return ServiceResult<int?>.Success(feedbackJob.Id);
    }

    public async Task<ServiceResult<AnalyzedJobDto?>> GetJobAnalysisByFeedbackId(int feedbackId, string userId)
    {
        var jobFeedback = await _dbContext.ResumeJobFeedbacks
            .Where(r => r.Id == feedbackId && r.UserId == userId)
            .Select(r =>
                new AnalyzedJobDto
                {
    ...
    public async Task<ServiceResult<List<JobFeedbackMinimalDto>>> GetAllJobFeedbacks(string userId,
        int? resumeId = null,
        int? jobId = null)
    {
        ...
        var result = await query.Select(f => new JobFeedbackMinimalDto
        {
            FeedbackId = f.Id,
            OverAllMatchScore = f.OverallMatchScore,
            ResumeId = f.ResumeId,
    ...
    public async Task<ServiceResult<List<JobWithBestMatchPreview>>> GetAllJobsWithBestMatch(string userId)
    {
        var result = await _dbContext.UserJobs
    ...
    public async Task<ServiceResult<List<JobWithFeedbacksPreview>>> GetJobsWithFeedbacks(string userId)
    {
        var result = await _dbContext.UserJobs
            .Where(j => j.UserId == userId)
            .Select(j => new JobWithFeedbacksPreview
    ...
    public async Task<ServiceResult<UpdateJobStatusDto?>> UpdateJobStatus(string userId, int jobId,
        UpdateJobStatusDto jobStatusDto)
    {
        ...
        return rowsAffected == 0
            ? ServiceResult<UpdateJobStatusDto?>.Failure("Job Not Found")
            : ServiceResult<UpdateJobStatusDto?>.Success(jobStatusDto);
    }

    public async Task<ServiceResult> UpdateJob(string userId, int jobId, JobSentDto jobUpdate)
    {
        var existingJob = await _dbContext.UserJobs.FirstOrDefaultAsync(j => j.UserId == userId && j.Id == jobId);
        if (existingJob == null)
        {
